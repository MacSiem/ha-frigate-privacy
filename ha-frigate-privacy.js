...
class HaFrigatePrivacy extends HTMLElement {
  constructor() {
    ...
    this._integrationInitStarted = false;
    this._integrationSyncInFlight = false;
  }
  ...
  set hass(hass) {
    ...
    if (!this._integrationInitStarted) {
      this._integrationInitStarted = true;
      this._connectIntegrationOnce();
    }
    if (!this._integrationChecked) {
  ...
  async _connectIntegrationOnce() {
    if (!this._hass || this._integrationSyncInFlight) return;
    this._integrationSyncInFlight = true;
    try {
      await this._callIntegration('get_state');
      this._integrationAvailable = true;
      this._integrationChecked = true;
      await this._migrateLegacySchedulesOnce();
      await this._refreshIntegrationState();
    } catch(e) {
      this._integrationAvailable = false;
      this._integrationChecked = true;
      console.info('[Frigate Privacy] Integration unavailable, using legacy localStorage mode:', e?.message || e);
      this._updateUI();
    } finally {
      this._integrationSyncInFlight = false;
    }
  ...
  async _migrateLegacySchedulesOnce() {
    const migratedKey = this._storageKey('schedules-migrated-v5');
    try {
      ...
      if (Array.isArray(schedules) && schedules.length > 0) {
        for (const schedule of schedules) {
          await this._callIntegration('set_schedule', { action: 'upsert', schedule });
        }
      }
      localStorage.setItem(migratedKey, '1');
    } catch(e) {
      console.warn('[Frigate Privacy] Schedule migration to integration failed:', e);
      throw e;
    }
  }

  async _refreshIntegrationState(existingState) {
    if (!this._integrationAvailable) return;
    try {
      const state = existingState || await this._callIntegration('get_state');
      if (Array.isArray(state.schedules)) {
        this._schedules = state.schedules;
      }
      if (Array.isArray(state.cameras) && state.cameras.length > 0 && !(this._config.cameras && this._config.cameras.length > 0)) {
        this._cameras = state.cameras.map(c => ({
          entity_id: c.entity_id || c.camera_id,
          name: c.name || c.camera_id
        }));
      }
      this._applyIntegrationPrivacyState(state.paused || {});
      this._updateUI();
    } catch(e) {
      console.warn('[Frigate Privacy] Could not refresh integration state:', e);
    }
  }

  _applyIntegrationPrivacyState(paused) {
    const active = Object.values(paused || {}).filter(item => item && item.active);
    if (active.length === 0) {
      this._privacyActive = false;
      this._privacyEndTime = null;
      this._privacyCameras = 'all';
    ...
    const endTime = first.ends_at ? Date.parse(first.ends_at) : null;
    const validFutureEnd = Number.isFinite(endTime) && endTime > Date.now();
    this._privacyActive = true;
    this._privacyEndTime = validFutureEnd ? endTime : null;
    this._privacyCameras = active.map(item => item.camera_entity_id || item.camera_id).join(', ');
    this._privacyStreamType = first.stream_type || 'all';
    this._warningSent = false;
    if (this._privacyEndTime) {
      this._startCountdown();
    } else if (this._privacyTimerInterval) {
      clearInterval(this._privacyTimerInterval);
      this._privacyTimerInterval = null;
    }
  }
  ...
  _loadSchedules() {
    this._loadSchedulesLegacy();
  }

  _loadSchedulesLegacy() {
    try {
      const raw = localStorage.getItem(this._storageKey('schedules'));
  ...
  _saveSchedules() {
    if (this._integrationAvailable) return;
    this._saveSchedulesLegacy();
  }

  _saveSchedulesLegacy() {
    // Legacy path: standalone card without the integration keeps schedules in this browser.
  ...
  async _setScheduleOnServer(schedule) {
    const result = await this._callIntegration('set_schedule', { action: 'upsert', schedule });
    return result?.schedule || schedule;
  }

  async _deleteScheduleOnServer(schedule) {
    if (!schedule?.id) return false;
    const result = await this._callIntegration('set_schedule', { action: 'delete', schedule_id: schedule.id });
  ...
  async _extendPrivacy(extraMinutes) {
    ...
    if (this._integrationAvailable) {
      try {
        const cams = this._privacyCameras && this._privacyCameras !== 'all'
          ? this._privacyCameras.split(', ')
          : this._cameras.map(c => c.entity_id);
        const remaining = Math.max(1, Math.ceil((this._privacyEndTime - Date.now()) / 60000));
        await this._callIntegration('pause_camera', {
          cameras: cams,
          duration_minutes: remaining,
          stream_type: this._privacyStreamType || 'all'
        });
      } catch(e) {
        console.warn('[Frigate Privacy] Could not extend integration pause:', e);
  ...
  async _pauseFrigate(minutes) {
    ...
    if (this._integrationAvailable) {
      try {
        const result = await this._callIntegration('pause_camera', {
          cameras: selectedCams,
          duration_minutes: parseInt(minutes),
          stream_type: streamType
        });
        const anyPaused = (result?.results || []).some(item => item.paused);
        if (!anyPaused) {
          throw new Error('No Frigate switches were paused');
        }
        this._privacyActive = true;
        ...
        this._applyIntegrationPrivacyState(result.state?.paused || {});
        this._startCountdown();
        ...
        console.warn('[Frigate Privacy] Integration pause failed:', e);
        this._showToast(t.errorStopAddon, 'error');
        return;
      }
    }
  ...
  async _resumeFrigate() {
    ...
    try {
      ...
      const camsToResume = this._privacyCameras && this._privacyCameras !== 'all'
        ...
        : this._cameras.map(c => c.entity_id);
      if (this._integrationAvailable) {
        const result = await this._callIntegration('resume_camera', { cameras: camsToResume });
        const blocked = (result?.results || []).some(item => item.fail_safe || item.resumed === false);
        if (blocked) {
          this._applyIntegrationPrivacyState(result.state?.paused || {});
  ...
  async _saveScheduleFromForm() {
    ...
    if (!this._integrationAvailable) await this._ensureHAHelpers();
    ...
    if (f.id) schedule.id = f.id;
    if (this._integrationAvailable) {
      try {
        schedule = await this._setScheduleOnServer(schedule);
      } catch(e) {
        console.warn('[Frigate Privacy] Could not save schedule via integration:', e);
        this._showToast(this._t.errorCallScript, 'error');
        return;
      }
    }
  ...
  async _deleteSchedule(idx) {
    const schedule = this._schedules[idx];
    if (this._integrationAvailable) {
      try {
        await this._deleteScheduleOnServer(schedule);
      } catch(e) {
        console.warn('[Frigate Privacy] Could not delete schedule via integration:', e);
        this._showToast(this._t.errorCallScript, 'error');
        return;
      }
    } else {
  ...
  async _toggleScheduleEnabled(idx) {
    this._schedules[idx].enabled = !this._schedules[idx].enabled;
    if (this._integrationAvailable) {
      try {
        this._schedules[idx] = await this._setScheduleOnServer(this._schedules[idx]);
      } catch(e) {
        console.warn('[Frigate Privacy] Could not toggle schedule via integration:', e);
        this._schedules[idx].enabled = !this._schedules[idx].enabled;
        this._showToast(this._t.errorCallScript, 'error');
        return;
      }
  ...
  _isIntroDismissed() {
    if (this._introDismissed) return true;
    try {
      return localStorage.getItem('ha-intro-dismissed-ha-frigate-privacy') === '1';
    } catch (_) {
      return false;
    }
  }
  ...
  _buildIntroBanner() {
    if (this._isIntroDismissed()) return '';
    const headline = this._lang === 'pl'
      ? 'Tryb prywatnosci Frigate: wstrzymaj detekcje, nagrywanie i migawki.'
      : 'Frigate privacy mode: pause detection, recording, and snapshots.';
    const steps = this._lang === 'pl'
      ? [
          'Wybierz kamery, zakres strumienia i czas, a nastepnie nacisnij przycisk Wstrzymaj.',
          'Zakladka Harmonogram ustawia codzienne okno prywatnosci.',
          'W kazdej chwili mozesz wznowic dzialanie kamer.',
        ]
      : [
          'Choose cameras, stream scope, and duration, then press Pause.',
          'Use Schedules to set a daily privacy window.',
          'Resume at any time to re-enable cameras.',
        ];
    return `<div class="intro-banner" data-intro="ha-frigate-privacy">
      <button class="intro-dismiss" data-action="dismiss-intro" type="button" title="Dismiss" aria-label="Dismiss">✕</button>
      <div class="intro-headline">💡 ${headline}</div>
      <ol class="intro-steps">${steps.map(step => `<li>${step}</li>`).join('')}</ol>
    </div>`;
  }

  _buildDonateFooter() {
    return `<div class="donate-section" data-source="ha-frigate-privacy">
  ...
  _dismissIntro() {
    ...
    try { localStorage.setItem('ha-intro-dismissed-ha-frigate-privacy', '1'); } catch (_) { /* ignore */ }
    this._updateUI();
  }
...
</style><style>${this._getCSS()}</style>
    <div class="card">
      ${this._buildIntroBanner()}
      <div class="header">
      ...
      ${this._buildDonateFooter()}
      <div class="toast"></div>
    
        </div>`;
	  }

	  _buildIntegrationHint() {
	    if (!this._integrationChecked || this._integrationAvailable) return '';
	    const t = this._t;
	    return `<div class="prereq-banner prereq-info">
	      <div class="prereq-icon" aria-hidden="true">\u24D8</div>
	      <div class="prereq-text"><strong>${t.integrationLegacyTitle}</strong><br>${t.integrationLegacyHint}</div>
	    </div>`;
	  }
...
# entity: input_boolean.frigate_privacy_mode</pre>
  ...
  _attachEvents() {
    ...
    const dismissIntro = sr.querySelector('[data-action="dismiss-intro"]');
    if (dismissIntro) {
      dismissIntro.addEventListener('click', (event) => {
